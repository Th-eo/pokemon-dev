...
class Trainer
  ...
  def trainertype
    @trainertype = @trainer_type if @trainertype
    return @trainertype
  end
  ...
  def initialize(name,trainertype)
    ...
    @pokegear=false
    @pokedex=false
    #clearPokedex #idk why this crashed, maybe it existed before
    #@shadowcaught=[]
...
class HallOfFame_Scene # Minimal change to store HoF time into a variable
  ...
  def writeTrainerData
    ...
    lefttext+=_INTL("Pokédex<r>{1}/{2}<br>",
        $player.pokedexOwned,$player.pokedexSeen)
